...
const hasPaymentsInfra = Boolean((process.env.TEST_DATABASE_URL ?? process.env.DATABASE_URL) && process.env.STRIPE_SECRET_KEY);

if (!hasPaymentsInfra) {
  test('payment race condition tests require DATABASE_URL (or TEST_DATABASE_URL) and STRIPE_SECRET_KEY', { skip: true }, () => {});
} else {
