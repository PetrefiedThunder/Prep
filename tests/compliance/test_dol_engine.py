...
def test_overtime_validation() -> None:
    ...
    assert all(v.timestamp.tzinfo == timezone.utc for v in overtime)
...
def test_minimum_wage_validation() -> None:
    ...
    assert all(v.timestamp.tzinfo == timezone.utc for v in minimum)
...
def test_child_labor_validation() -> None:
    ...
    assert all(v.timestamp.tzinfo == timezone.utc for v in child_labor)
...
def test_record_keeping_validation() -> None:
    ...
    data = {
        "payroll_records": [
            ...
            {"id": "rec3", "created_date": datetime.now(timezone.utc)},
        ]
    }
    ...
    assert len(record) == 2
    assert all(v.timestamp.tzinfo == timezone.utc for v in record)
...
def test_break_validation() -> None:
    ...
    assert len(breaks) == 1
    assert all(v.timestamp.tzinfo == timezone.utc for v in breaks)
