...
class GDPRCCPAEngine(ComplianceEngine):
    ...
    async def check_consent_management(self, entity_data: Dict[str, Any]) -> PrivacyCheckResult:
        ...
        return PrivacyCheckResult(
            ...
            details="Consent records include timestamps" if passed else "Incomplete consent records",
            remediation=remediation,
        )

    async def check_data_retention_policies(self, entity_data: Dict[str, Any]) -> PrivacyCheckResult:
        ...
        return PrivacyCheckResult(
            ...
            passed=passed,
            severity="medium",
            details="Retention policy documented" if passed else "Missing retention policy documentation",
            remediation=remediation,
        )
    ...
    async def check_privacy_notices(self, entity_data: Dict[str, Any]) -> PrivacyCheckResult:
        ...
        return PrivacyCheckResult(
            ...
            passed=passed,
            severity="medium",
            details="Localized privacy notices available" if passed else "Jurisdictional coverage missing",
            remediation=remediation,
        )
