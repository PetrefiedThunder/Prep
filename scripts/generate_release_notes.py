...
def generate_release_notes(from_tag: str, to_tag: str):
    ...
    for category, pr_list in sorted(categories.items()):
        ...
        for pr in pr_list:
            ...
            title = re.sub(r'^(feat|fix|docs|perf|refactor|security):\s*', '', title, flags=re.IGNORECASE)
    ...
    notes.append("---")
    notes.append("")
    notes.append(f"**Full Changelog**: https://github.com/PetrefiedThunder/Prep/compare/{from_tag}...{to_tag}")
